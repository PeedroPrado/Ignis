...
const Mapa: React.FC<MapaProps> = ({ tipo, filtros }) => {
  ...
  const montarQueryParams = useCallback(() => {
    ...
    if (filtros.estado) params.append('estado_id', filtros.estado);
    if (filtros.bioma) params.append('bioma_id', filtros.bioma);

    if (tipo === 'risco') {
      // 🔥 Risco usa só 'data'
      if (filtros.inicio) params.append('data', filtros.inicio);
    } else {
      // 🔥 Foco de Calor e Área Queimada continuam com 'inicio' e 'fim'
